...
class Game extends Component{
  ...
  MakeRow=(posx)=>{
    ...
    for(let i = 0;i<this.state.GridY;i++){
      const b = (<button onClick={()=>!this.state.dragbool &&this.BoardInput(posx.posx,i)} onTouchStart={()=>setTimeout(()=>{if(!this.state.dragbool){this.BoardInput(posx.posx,i)}},200)} id={"GridButton"+((i+1)+((posx.posx)*this.state.GridX))} className={`${this.checks.newtilebool(this.state.GridLetters,this.state.storedgrid,posx.posx,i)?`dark:text-fuchsia-300 text-sky-500`:`dark:text-white text-slate-950`} transition w-19 h-19 ease-in-out hover:scale-110 dark:bg-stone-900  dark:border-stone-900 bg-white border active:ring dark:active:ring-fuchsia-500 active:ring-sky-500 rounded-md lg:rounded-lg aspect-square text-5xl`}>{this.state.GridLetters[posx.posx][i]}</button>);
      rowbuttons.push(b)
    }
    return(
      rowbuttons
      ...
      <div className="w-full h-full flex justify-center items-center">
        <Draggable onDrag={this.eventControl} onStop={this.eventControl}>
          <div className="flex flex-col justify-center gap-2">
            {(this.state.ExpandPoint>0)?<button className=" flex justify-center self-center transition w-full h-19 ease-in-out hover:border-lime-400 border-2 dark:hover:border-orange-300 dark:border-stone-950 border-zinc-50 rounded-t-full "  onClick={()=>!this.state.dragbool &&this.ExpandGrid("up")}><ArrowUpCircleIcon onTouchStart={()=>setTimeout(()=>{if(!this.state.dragbool){this.ExpandGrid("up")}},200)} className=" h-14 w-14 dark:fill-orange-300 fill-lime-400 overflow-hidden "></ArrowUpCircleIcon></button>:null}
            <div className="flex flex-row justify-center gap-2">
              {(this.state.ExpandPoint>0)?<button className="transition w-full h-19 ease-in-out hover:border-lime-400 dark:hover:border-orange-300 border-2 dark:border-stone-950 border-zinc-50 rounded "  onClick={()=>!this.state.dragbool &&this.ExpandGrid("left")}><ArrowLeftCircleIcon onTouchStart={()=>setTimeout(()=>{if(!this.state.dragbool){this.ExpandGrid("left")}},200)} className="  h-14 w-14 dark:fill-orange-300 fill-lime-400 overflow-hidden"></ArrowLeftCircleIcon></button>:null}
              <this.MakeGrid/>
              {(this.state.ExpandPoint>0)?<button className="transition w-full h-19 ease-in-out hover:border-lime-400 dark:hover:border-orange-300 border-2 rounded dark:border-stone-950 border-zinc-50"  onClick={()=>!this.state.dragbool &&this.ExpandGrid("right")}><ArrowRightCircleIcon onTouchStart={()=>setTimeout(()=>{if(!this.state.dragbool){this.ExpandGrid("right")}},200)}  className=" h-14 w-14 dark:fill-orange-300 fill-lime-400 overflow-hidden"></ArrowRightCircleIcon></button>:null}
            </div>
            {(this.state.ExpandPoint>0)?<button className=" flex justify-center transition w-full h-19 ease-in-out hover:border-lime-400 dark:hover:border-orange-300 border-2 rounded-b-full dark:border-stone-950 border-zinc-50"  onClick={()=>!this.state.dragbool &&this.ExpandGrid("down")}><ArrowDownCircleIcon onTouchStart={()=>setTimeout(()=>{if(!this.state.dragbool){this.ExpandGrid("down")}},200)} className=" dark:fill-orange-300  h-14 w-14 fill-lime-400 overflow-hidden" ></ArrowDownCircleIcon></button>:null}
          </div>
        </Draggable>
      </div>
