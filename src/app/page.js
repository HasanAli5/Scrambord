...
class Game extends Component{
  ...
  MakeRow=(posx)=>{
    ...
    for(let i = 0;i<this.state.GridY;i++){
      const b = (<button onClick={()=>!this.state.dragbool &&this.BoardInput(posx.posx,i)} onTouchStart={()=>setTimeout(()=>{if(!this.state.dragbool){this.BoardInput(posx.posx,i)}},101)} id={"GridButton"+((i+1)+((posx.posx)*this.state.GridX))} className={`${this.checks.newtilebool(this.state.GridLetters,this.state.storedgrid,posx.posx,i)?`dark:text-fuchsia-300 text-sky-500`:`dark:text-white text-slate-950`} transition w-19 h-19 ease-in-out hover:scale-110 dark:bg-stone-900  dark:border-stone-900 bg-white border active:ring dark:active:ring-fuchsia-500 active:ring-sky-500 rounded-md lg:rounded-lg aspect-square text-5xl`}>{this.state.GridLetters[posx.posx][i]}</button>);
      rowbuttons.push(b)
    }
    return(
      rowbuttons
