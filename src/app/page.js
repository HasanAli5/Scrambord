...
import {ArrowUpOnSquareIcon,QuestionMarkCircleIcon,ChartBarIcon, AcademicCapIcon} from '@heroicons/react/24/outline';
...
class Game extends Component{
  ...
  constructor(props){
    ...
    this.state = {
      OriginalDate :new Date().setUTCFullYear(2023,8-1,25),
      username:"Player",
      gamestart:true,
      dragbool:false,
      showdono:false,
...
//grid props
  ...
  DrawnHand=()=>{
    let gridbutton = []
    for(let i =0;i<10;i++){
      const b = (<button disabled={this.state.disabledButtons[i]} onClick={()=>!this.state.dragbool &&this.HandInput(i)} onTouchStart={()=>!this.state.dragbool &&this.HandInput(i)} id={"HandButton"+i} className={`${i==this.state.SelectedLetterIndex?` border-sky-300 dark:border-fuchsia-400 border-2 border-4`:`border dark:border-stone-700`} overflow-hidden disabled:hover:scale-100  dark:bg-stone-900 dark:text-white disabled:opacity-25 disabled:bg-neutral-400 grow h-full transition ease-in-out hover:scale-110 bg-white  text-slate-950 rounded-md lg:max-2xl:rounded-lg text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl 2xl:text-6xl`}>{this.state.HandLetters[i]}</button>);
      gridbutton.push(b)
    }
    return (gridbutton)
  }
  ...
  TogglePopupInfo =()=>{
    if(this.state.showinfopop==false){
  ...
  ToggleSummary =()=>{
    if(this.state.summarypop==false){
      this.setState({summarypop:true});
    }
    else{
      this.setState({summarypop:false});
    }
  }
  ToggleDark =()=>{
  ...
  InfoPopup=()=>{
      return(
        <div className="z-20 fixed inset-0 flex flex-col justify-center items-center bg-black w-full h-full bg-opacity-25">
          <div className=" w-80 relative dark:bg-stone-900 bg-white rounded-md p-3">
          <button onTouchStart={()=>!this.state.dragbool &&this.TogglePopupInfo()} onClick={()=>!this.state.dragbool &&this.TogglePopupInfo()} className="absolute top-0 left-0 h-10 transition ease-in-out dark:bg-stone-900 bg-white text-slate-950 rounded-md lg:max-2xl:rounded-lg aspect-square text-lg sm:text-xl md:text-2xl lg:text-3xl"><XMarkIcon className=" dark:stroke-white dark:fill-white stroke-1 "/></button>
          <h1 className="text-6xl pt-14 pb-3 dark:text-white text-center">How To Play</h1>
          <p className=" text-center text-xl dark:text-white py-2">Place down letters from your hand to create words on the board.</p>
          <p className=" text-center text-xl dark:text-white py-2">Submit board using the <ArrowUpOnSquareIcon className="m-1 p-1 w-10 inline border rounded dark:border-stone-700"/> button. If valid then new letter will be added to hand.</p>
          <p className=" text-center text-xl dark:text-white py-2">Expansion Points (ExP) can be used to reduce Strikes (Click Strikes Bar) or Expand (Arrows).</p>
          <p className=" text-center text-xl dark:text-white py-2">Everyday has unique order of letters dealt so enjoy day {Math.ceil(Math.abs(this.date-this.state.OriginalDate)/(86400000))} !</p>
          </div>
        </div>
    )
  }
  ...
  SummaryPopUp=()=>{
    var days = Math.ceil(Math.abs(this.date-this.state.OriginalDate)/(86400000));
    var rounds =this.state.Round;
    var score =this.checks.Evalute(this.state.GridLetters)[1];
    return(
      <div className="z-20 fixed inset-0 flex flex-col justify-center items-center bg-black w-full h-full bg-opacity-25">
        <div className=" w-80 relative dark:bg-stone-900 bg-white rounded-md p-3">
        <button onTouchStart={()=>!this.state.dragbool &&this.ToggleSummary()} onClick={()=>!this.state.dragbool &&this.ToggleSummary()} className="absolute top-0 left-0 h-10 transition ease-in-out dark:bg-stone-900 bg-white text-slate-950 rounded-md lg:max-2xl:rounded-lg aspect-square text-lg sm:text-xl md:text-2xl lg:text-3xl"><XMarkIcon className=" dark:stroke-white dark:fill-white stroke-1 "/></button>
        <h1 className="text-6xl pt-14 pb-2 dark:text-white text-center">Summary</h1>
        ...
        <p className=" text-center text-2xl dark:text-white py-2">Day : {days}</p>
        ...
        {!this.state.gamestart&&this.state.ExpandPoint==0?<div className="flex justify-center"><button className=" hover:focus-visible:border-lime-400  transition ease-in-out hover:scale-110 inset-x-0 border dark:border-stone-700 rounded-full text-3xl self-center px-3 m-1 dark:text-white" onClick={()=>SubmitScore(AES.encrypt(JSON.stringify({"days":days,"rounds":rounds,"score":score,"username":this.state.username}),this.dateinput).toString())} onTouchStart={()=>SubmitScore(AES.encrypt(JSON.stringify({"days":days,"rounds":rounds,"score":score,"username":this.state.username}),this.dateinput).toString())}>Submit Score</button></div>:null}
        </div>
      </div>
  )
}
  ...
  componentDidMount(){
    try{
      this.setState(JSON.parse(AES.decrypt(localStorage.getItem("st"),this.dateinput+localStorage.getItem("randid")).toString(CryptoJS.enc.Utf8)));
    }catch(e){
      localStorage.removeItem("st");
    }
    try{//add later
      let jsonuser=Object.values(JSON.parse(AES.decrypt(localStorage.getItem("c"),localStorage.getItem("randid")).toString(CryptoJS.enc.Utf8)));
  ...
  componentDidUpdate(){
    localStorage.setItem("st",AES.encrypt(JSON.stringify(this.state),this.dateinput+localStorage.getItem("randid")).toString());
    localStorage.setItem("c",AES.encrypt(JSON.stringify({"username":this.state.username,"caps":this.caps}),localStorage.getItem("randid")).toString());
  }
  ...
  render() {
    ...
    <>
      <div className="flex flex-row w-full z-10 h-1/16 fixed top-0 dark:bg-stone-900 dark:border-stone-700 bg-white border justify-center">
        <button className="flex flex-row transition ease-in-out hover:scale-110 m-1 bg-white border dark:bg-stone-900 dark:border-stone-700  rounded-md sm:rounded-sm aspect-square md:rounded-md lg:rounded-lg" onClick={()=>!this.state.dragbool &&this.TogglePopupInfo()} onTouchStart={()=>!this.state.dragbool &&this.TogglePopupInfo()}>
          <QuestionMarkCircleIcon className="m-1  dark:stroke-white"/>
        </button>
        <button className="transition ease-in-out hover:scale-110 m-1 bg-white border text-slate-950 dark:bg-stone-900 dark:border-stone-700 dark:text-white rounded-md sm:rounded-sm md:rounded-md lg:rounded-lg aspect-square h-auto"onClick={()=>!this.state.dragbool &&this.ToggleSummary()} onTouchStart={()=>!this.state.dragbool &&this.ToggleSummary()}>
        ...
        <button className="flex flex-row transition ease-in-out hover:scale-110 m-1 bg-white border dark:bg-stone-900 dark:border-stone-700 rounded-md sm:rounded-sm aspect-square md:rounded-md lg:rounded-lg" onClick={()=>!this.state.dragbool &&this.TogglePopupDono()} onTouchStart={()=>!this.state.dragbool &&this.TogglePopupDono()}>
